...
async function salvar_datos(){
    ...
    if (validacion_datos==1){
        mensaje_error='Se encontraron campos vacios.'; 
        alert(mensaje_error);
    } else {
        $("#ventana_agregar_Pregunta").show();
        $("#cerrar_ventana").hide();
        await salvar_registro_con_fotos(descripcion,sugerencias,respuesta);
...
async  function salvar_registro_con_fotos (descripcion,sugerencias,respuesta){
...
function validar_campos(descripcion,sugerencias,respuesta){
    var validar=0;
