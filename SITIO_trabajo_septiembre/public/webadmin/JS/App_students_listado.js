...
function llenarMaterias(res){
	...
	res.forEach(rpta=>{
		...
		$('#datos_alumnos').append("<tr><td>" +  nombre   + "</td><td>" +  email  + "</td><td><button type='button' class='btn btn-dark text-center' data-toggle='modal' data-target='#modificar_alumnos'   onclick='modificar(\"" + clave + "\");'>Modificar</button></td><td><button type='button' class='btn btn-dark text-center'  data-toggle='modal' data-target='#borrar_alumnos'  onclick='borrar(\"" + clave + "\");'>Borrar</button></td></tr>");
        //console.log("<tr><td>" +  nombre   + "</td><td>" +  email  + "</td><td>" +  mensaje + "</td><td><button type='button' class='btn btn-dark text-center'  onclick='modificar(" + KEY + ");'>Modificar</button></td><td><button type='button' class='btn btn-dark text-center'  onclick='borrar(" + KEY + ");'>Borrar</button></td></tr>");
...
$("#eliminar").on('click', function(evt){
    firebase.firestore().collection('students').doc(id_alumno).delete().then(res=>{
        location.reload();
    }).catch(err=>{
        // en caso de error
    });
...
$("#actualizar").on('click', function(evt){
    ...
    var idAlumno=($("#clave_alumno2").val()).trim();
...
function llenar_formulario(alumno_seleccionado){
    ...
    firebase.firestore().collection('students').doc(alumno_seleccionado).get().then(res=>{
      var datos=res.data();
      $("#clave_alumno2").val(alumno_seleccionado);
